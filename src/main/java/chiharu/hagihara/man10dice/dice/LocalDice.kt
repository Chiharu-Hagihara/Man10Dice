...
object LocalDice {
    ...
    fun localDice(p: Player, number: String) {
        ...
        p.sendMsg("&l${p.name}がダイスを振っています・・・&k&lxx")

        for (i in 0 until players.size) {
            players[i].sendMsg("&l${p.name}がダイスを振っています・・・&k&lxx")
        }

        object : BukkitRunnable() {
            override fun run() {
                p.sendMsg("&3&l${p.name}&3&lは&e&l${number}&3&l面サイコロを振って&e&l${result}&3&lが出た")

                for (i in 0 until players.size) {
                    players[i].sendMsg("&3&l${p.name}&3&lは&e&l${number}&3&l面サイコロを振って&e&l${result}&3&lが出た")
                }
